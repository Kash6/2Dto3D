...
class stereo_vision:
    ...
    def __init__(self, so_lib_path=DEFAULT_STEREO_VISION_SO_PATH, width=1242, height=375, 
                ...
                CAMERA_CALIBRATION_YAML='calibration/Kitti_Color.yml',
                subsampling = False):
        self.sv = ctypes.CDLL(so_lib_path)
        self.width = width
        self.height = height
        self.sv.generatePointCloud.restype = ndpointer(dtype = ctypes.c_double, shape=(width*height,3))
        self.sv.getColor.restype = ndpointer(dtype = ctypes.c_ubyte, shape=(width*height,4))
        ...
        self.sv.generatePointCloud.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_bool, ctypes.c_bool, ctypes.c_bool, ctypes.c_bool, ctypes.c_int, ctypes.c_int, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p, ctypes.c_bool, ctypes.c_bool]
...
def main():
    ...
    kittiPath = args.kitti
    scale_factor = args.scale
    pc_extrapolation = args.pointcloud_interpolation
    ...
    if args.demo:
        ...
        for iFrame in range(len(os.listdir(f"{kittiPath}/video/testing/image_02/0000/"))):
            ...
            left = cv2.resize(left, (left.shape[1]//scale_factor, left.shape[0]//scale_factor))
            right = cv2.resize(right, (right.shape[1]//scale_factor, right.shape[0]//scale_factor))
            pc, colorMap = s.generatePointCloud(left, right)
            np.save(f"./PointClouds/{iFrame}", np.array(pc), allow_pickle=True, fix_imports=True)
            np.save(f"./PointClouds/{iFrame}_ColorMap", np.array(colorMap), allow_pickle=True, fix_imports=True)
